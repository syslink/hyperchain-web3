...
const ChainIdPrefix = 'ChainProvider-';
let KeyStoreFileName = 'keystoreInfo';
...
function getDataFromFile(chainProvider) {
  if (chainProvider == null) {
    chainProvider = utils.getProvider();
  }
...
export function init(password) {
  ...
  if (keystoreInfo == null) {
    ...
    generateAccount(mnemonicWords, password);
...
export function initByMnemonic(mnemonicWords, password) {
  ...
  wallet.encrypt(password, null).then((ksInfoStr) => {
    ...
    ksInfoObj['publicKey'] = utils.checkPrefix(publicKey);
